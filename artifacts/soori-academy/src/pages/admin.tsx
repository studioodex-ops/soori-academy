...
type Student = {
  id: number; name: string; phone: string; email?: string; country?: string;
  batch?: string; status: string; notes?: string; createdAt: string;
  paymentStatus?: string; paymentId?: number; receiptData?: string;
};
type ZoomSession = {
  id: number; batch: string; weekNumber: number; title?: string;
...
function LoginScreen({ onLogin }: { onLogin: (t: string) => void }) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      setErr("Connection error. Is the API server running?");
    } finally {
      setLoading(false);
    }
  }
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <Input
              ...
              className="h-11 rounded-xl text-white border"
              style={{ background: "#18191a", borderColor: "#3a3b3c" }}
            />
          </div>
          {err && <p className="text-red-400 text-sm">{err}</p>}
          <Button type="submit" disabled={loading} className="w-full h-11 font-bold rounded-xl text-white border-0"
            style={{ background: "#1877F2" }}>
            {loading ? "Signing in…" : "Sign In"}
          </Button>
        </form>
        <p className="text-center text-xs mt-4" style={{ color: "#65676b" }}>
          Default password: <span className="font-mono text-white">soori2024</span>
        </p>
      </motion.div>
    </div>
  );
...
function StudentModal({ student, token, onClose, onRefresh }: {
  ...
  const [messages, setMessages] = useState<{ id: number; message: string; sentAt: string }[]>([]);
